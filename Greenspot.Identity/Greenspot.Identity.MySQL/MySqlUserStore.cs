...
namespace Greenspot.Identity.MySQL
{
    ...
    where TUser : IdentityUser
    {
        ...
        private readonly UserSnsInfoRepository<TUser> _userSnsInfoRepository;

        public MySqlUserStore()
        {
            new MySqlUserStore<TUser>(new MySqlDatabase());
        }

        public MySqlUserStore(MySqlDatabase database)
        {
            Database = database;
            _userRepository = new UserRepository<TUser>(database);
            _userLoginRepository = new UserLoginRepository(database);
            _userClaimRepository = new UserClaimRepository<TUser>(database);
            _userRoleRepository = new UserRoleRepository<TUser>(database);
            _userSnsInfoRepository = new UserSnsInfoRepository<TUser>(database);
        }
        ...
        public Task<TUser> FindByPhoneNumberAsync(string phone)
        {
            if (phone == null)
